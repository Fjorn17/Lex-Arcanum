...
| `<lang>/pages/astronomy/` | The whole art in one section, behind one nav tab. 29 pages for Astronomy and the Disciplines inside it, 302 spell pages, and an index that carries both: the tree at the top and the filterable table of every spell below. Nothing here is edited by hand except the seven spells written from scratch: the Discipline pages come from `doc/spells/build-disciplines.pl` and the rest from `doc/spells/build-pages.pl`. |
