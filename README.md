...
| `<lang>/pages/spells/` | One page per spell: the seven written for the supplement plus the 339 of the SRD 5.2.1, and the index table, which can be filtered and sorted by level, school and class. The SRD pages are generated by `doc/spells/build-pages.pl` and are not edited by hand. |
...
- **Spells** — the seven original ones have pages (wording and numbers still drafts), and the 339 spells of the SRD 5.2.1 are imported into both trees, with the English text from the SRD and a Spanish translation of our own using the PHB 2024 vocabulary.
